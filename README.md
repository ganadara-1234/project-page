...
- Mesh: unchanged `../gen_figure/main/dream_final_r035_lr12e3_30k/full_merged_mesh.ply`, copied to `static/models/dream03.ply` (102,644 vertices / 180,000 triangles). The initial direction and up vector come from this figure's `map/selected_view_manifest.json`. The poster is its `map/selected_view_clean_cropped.png`, exported to WebP. The viewer preserves the PLY vertex colors and renders a triangle mesh, not Gaussians.
